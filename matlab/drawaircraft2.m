function drawaircraft2(uu)
    ...
    % --- NEW PERSISTENT VARIABLES FOR TRACE ---
    persistent trace_handle
    persistent trace_data
    
    if t == 0 
        figure(2), clf
        [vertices, faces, facecolors] = definemonocopter();
        
        % Initialize Trace Data (Empty)
        trace_data = [];
        ...
        % We initialize it as an empty line. 'k:' means Black Dotted line.
        trace_handle = plot3(0, 0, 0, 'k:', 'LineWidth', 1.5);
        ...
        limit = 15;
        axis([-limit, limit, -limit, limit, 10, 25]);
            
    else
        % --- UPDATE TRACE DATA ---
        % Your visualization rotates the world:
        % Plot X = Real East (pe)
        % Plot Y = Real North (pn)
        % Plot Z = -Real Down (-pd)
        new_point = [pe, pn, -pd];
        
        % Append new point to history
        trace_data = [trace_data; new_point];
        
        % Update the line object on the plot 
        set(trace_handle, 'XData', trace_data(:,1), ...
                          'YData', trace_data(:,2), ...
                          'ZData', trace_data(:,3));
